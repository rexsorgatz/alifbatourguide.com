...
- `/build.py` — Builds `index.html` from `content.html` + template; also regenerates `sitemap.xml`
...
All pages link Typekit CSS via the external CDN: `https://use.typekit.net/qlf7wqv.css`. A local copy exists at `/css/qlf7wqv.css` but is not used.
